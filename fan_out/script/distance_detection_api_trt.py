...
class InferenceRequest(BaseModel): 
    ...
    visualize_mode: Optional[bool] = False 
...
class InferenceResponse(BaseModel): 
    ...
    visualized_folder: Optional[str] = None 
...
class DistanceDetectionInference: 
    ...
    def _process_batch(self, batch_paths: List[Path], visualize_mode: bool = False) -> List[Dict[str, Any]]:
        ...
        for i, original_idx in enumerate(valid_indices):
            ...
            if center_boxes.shape[0] == 0:
                ...
                # Visualization for no center found
                if visualize_mode:
                    vis_img = img.copy()
                    for box in pad_boxes:
                        cv2.rectangle(vis_img, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (255, 0, 0), 2)
                    vis_dir = p.parent / "visualized_results"
                    vis_dir.mkdir(parents=True, exist_ok=True)
                    cv2.imwrite(str(vis_dir / p.name), vis_img)
                continue
            ...
            min_d = float('inf')
            for box in pad_boxes:
                dx, dy, sp, ep = DistanceDetectionInference.calculate_distance(center_box, box)
            ...
            if visualize_mode:
                vis_img = img.copy()
                # Draw Center
                cv2.rectangle(vis_img, (int(center_box[0]), int(center_box[1])), (int(center_box[2]), int(center_box[3])), (0, 255, 0), 2)
                cv2.putText(vis_img, "center", (int(center_box[0]), int(center_box[1]) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                ...
                # Draw distance line
                if best_points and min_center_to_pad_distance is not None:
                    sp, ep = best_points
                    cv2.line(vis_img, sp, ep, (0, 0, 255), 2)
                    cv2.putText(vis_img, f"dist: {min_center_to_pad_distance}", (max(sp[0], ep[0]), max(sp[1], ep[1])), 
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                
                vis_dir = p.parent / "dis_visualized_results"
                vis_dir.mkdir(parents=True, exist_ok=True)
                cv2.imwrite(str(vis_dir / p.name), vis_img)

        return [results_map[str(p)] for p in batch_paths]

    def process_folder(self, job_folder: Path, exts: Optional[List[str]] = None, batch_size: int = 8, visualize_mode: bool = False) -> List[Dict[str, Any]]: 
        if exts is None: 
        ...
        for i in range(0, len(images), batch_size):
            ...
            results.extend(self._process_batch(batch_paths, visualize_mode=visualize_mode))
...
def inference(req: InferenceRequest) -> InferenceResponse: 
    ...
    t0 = time.time() 
    results = server_engine.process_folder(folder, req.image_extensions, visualize_mode=req.visualize_mode) 
    ...
    vis_folder_str = str(folder / "visualized_results") if req.visualize_mode else None
    ...
    payload = { 
        ...
        "visualized_folder": vis_folder_str,
    } 
    return InferenceResponse(**payload) 
